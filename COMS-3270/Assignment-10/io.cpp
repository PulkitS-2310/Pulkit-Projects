...
#define POKEMART_POTION_PRICE 100
#define POKEMART_REVIVE_PRICE 100
#define POKEMART_POKEBALL_PRICE 100
...
static void io_buy_item(const char *name, int price, int &quantity)
{
  if (world.pc.pokebucks < price) {
    battle_pause_message(std::string("You do not have enough PokeBucks for a ") +
                         name + ".");
    return;
  }

  world.pc.pokebucks -= price;
  quantity++;

  battle_pause_message(std::string("Bought one ") + name + " for " +
                       std::to_string(price) + " PokeBucks.");
}
...
void io_pokemart()
{
  while (1) {
    ...
    mvprintw(1, 0, "Money: %d PokeBucks", world.pc.pokebucks);
    mvprintw(3, 0, "1. Potion   - %d PokeBucks", POKEMART_POTION_PRICE);
    mvprintw(4, 0, "2. Revive   - %d PokeBucks", POKEMART_REVIVE_PRICE);
    mvprintw(5, 0, "3. Pokeball - %d PokeBucks", POKEMART_POKEBALL_PRICE);
    mvprintw(7, 0, "Inventory: Potions %d  Revives %d  Pokeballs %d",
             world.pc.potions, world.pc.revives, world.pc.pokeballs);
    mvprintw(9, 0, "Press 1-3 to buy, or ESC to leave.");
    refresh();
    ...
    if (key == 27) {
      ...
      io_buy_item("Potion", POKEMART_POTION_PRICE, world.pc.potions);
    } else if (key == '2') {
      io_buy_item("Revive", POKEMART_REVIVE_PRICE, world.pc.revives);
    } else if (key == '3') {
      io_buy_item("Pokeball", POKEMART_POKEBALL_PRICE, world.pc.pokeballs);
    }
  }
}

static void io_pokemon_storage_menu()
{
  int key;

  while (1) {
    clear();
    mvprintw(0, 0, "Pokemon Center Storage");
    mvprintw(1, 0, "Deposit with 1-6. Withdraw stored Pokemon with a-j. ESC exits.");

    mvprintw(3, 0, "Party:");
    for (size_t i = 0; i < world.pc.party.size(); i++) {
      mvprintw(static_cast<int>(i) + 4, 0,
               "%zu. %-14s Lv:%3d HP:%3d/%3d %s",
               i + 1,
               world.pc.party[i].name.c_str(),
               world.pc.party[i].level,
               world.pc.party[i].current_hp,
               world.pc.party[i].max_hp,
               static_cast<int>(i) == world.pc.active_pokemon ? "(active)" : "");
    }

    mvprintw(11, 0, "Storage: %zu Pokemon", world.pc.storage.size());
    size_t visible = world.pc.storage.size() < 10 ? world.pc.storage.size() : 10;
    for (size_t i = 0; i < visible; i++) {
      mvprintw(static_cast<int>(i) + 12, 0,
               "%c. %-14s Lv:%3d HP:%3d/%3d",
               static_cast<char>('a' + i),
               world.pc.storage[i].name.c_str(),
               world.pc.storage[i].level,
               world.pc.storage[i].current_hp,
               world.pc.storage[i].max_hp);
    }

    if (world.pc.storage.size() > visible) {
      mvprintw(22, 0, "Only the first 10 stored Pokemon are shown.");
    }
    ...
    if (key >= '1' && key <= '6') {
      ...
      if (idx >= static_cast<int>(world.pc.party.size())) {
        continue;
      }
      if (world.pc.party.size() <= 1) {
        battle_pause_message("You must keep at least one Pokemon in your party.");
        continue;
      }

      world.pc.storage.push_back(world.pc.party[idx]);
      world.pc.party.erase(world.pc.party.begin() + idx);

      if (world.pc.active_pokemon == idx) {
        world.pc.active_pokemon = first_usable_pokemon(world.pc.party);
        if (world.pc.active_pokemon < 0) {
          world.pc.active_pokemon = 0;
        }
      } else if (world.pc.active_pokemon > idx) {
        world.pc.active_pokemon--;
      }

      battle_pause_message("Pokemon deposited in storage.");
    } else if (key >= 'a' && key <= 'j') {
      int idx = key - 'a';

      if (idx >= static_cast<int>(world.pc.storage.size())) {
        continue;
      }
      if (world.pc.party.size() >= 6) {
        battle_pause_message("Your party is full. Deposit a Pokemon first.");
        continue;
      }

      world.pc.party.push_back(world.pc.storage[idx]);
      world.pc.storage.erase(world.pc.storage.begin() + idx);
      battle_pause_message("Pokemon withdrawn to your party.");
    }
  }
}
...
void io_pokemon_center()
{
  ...
  heal_party(world.pc.storage);

  while (1) {
    clear();
    mvprintw(0, 0, "Welcome to the Pokemon Center!");
    mvprintw(1, 0, "Your Pokemon have been fully healed.");
    mvprintw(3, 0, "Party Pokemon: %zu/6", world.pc.party.size());
    mvprintw(4, 0, "Stored Pokemon: %zu", world.pc.storage.size());
    mvprintw(6, 0, "S. Manage party/storage");
    mvprintw(7, 0, "ESC. Leave");
    refresh();
                                ...
                                battle_state_t &state)
{
  ...
  battle_pause_message(player_pokemon.name + " used " +
  ...
  return;
}
    {
      ...
      double effectiveness =
        type_effectiveness_multiplier(enemy_pokemon,
                                      player_pokemon.moves[action.move_index]);
      std::string msg = player_pokemon.name + " used " +
                        move_name(player_pokemon.moves[action.move_index]) +
                        " for " + std::to_string(damage) + " damage!" +
                        type_effectiveness_message(effectiveness);
  ...
  case battle_action_pokeball:
    ...
    if (!attempt_pokemon_capture(enemy_party[enemy_active])) {
      battle_pause_message("The wild " + enemy_party[enemy_active].name +
                           " broke free!");
      break;
    }
    ...
    if (world.pc.party.size() < 6) {
      ...
      battle_pause_message("You caught the wild " +
                           enemy_party[enemy_active].name + "!");
    } else {
      world.pc.storage.push_back(enemy_party[enemy_active]);
      battle_pause_message("You caught the wild " +
                           enemy_party[enemy_active].name +
                           "! It was sent to Pokemon Center storage.");
    }
  ...
  case battle_action_move:
    {
      ...
      double effectiveness =
        type_effectiveness_multiplier(player_pokemon,
                                      enemy_pokemon.moves[action.move_index]);
                           ...
                           npc *trainer)
{
  ...
  if (!state.player_won && !state.player_ran && !state.wild_caught && !state.wild_fled) {
    ...
    world.pc.pokebucks += reward;

    battle_pause_message("You defeated the trainer! You earned " +
                         std::to_string(reward) + " PokeBucks.");
  }
