  ...
  for (y = 0; y < MAP_Y; y++) {
    for (x = 0; x < MAP_X; x++) {
      ...
      if (world.cur_map->cmap[y][x]) {
        ...
        if (world.cur_map->cmap[y][x] == &world.pc) {
          attron(COLOR_PAIR(7)); 
        } else {
          attron(COLOR_PAIR(6)); 
        }
        ...
        case ter_clearing:
          attron(COLOR_PAIR(2));
          mvaddch(y, x,
            (world.cur_map->map[y][x] == ter_tree) ? TREE_SYMBOL :
            (world.cur_map->map[y][x] == ter_forest) ? FOREST_SYMBOL :
            (world.cur_map->map[y][x] == ter_grass) ? TALL_GRASS_SYMBOL :
            SHORT_GRASS_SYMBOL);
        ...
        case ter_gate:
          attron(COLOR_PAIR(3));
          mvaddch(y, x,
            (world.cur_map->map[y][x] == ter_path) ? PATH_SYMBOL :
            GATE_SYMBOL);
        ...
        case ter_center:
          attron(COLOR_PAIR(5));
          mvaddch(y, x,
            (world.cur_map->map[y][x] == ter_mart) ? POKEMART_SYMBOL :
...
void game_loop() {
  ...
  while (1) {
    ...
    if (c == &world.pc) {
      ...
      int key = getch();
      int NewY = c->pos[dim_y];
      int NewX = c->pos[dim_x];

      switch (key) {
        case '8': case 'k': NewY--; break;
        case '2': case 'j': NewY++; break;
        case '4': case 'h': NewX--; break;
        case '6': case 'l': NewX++; break;
        case '7': case 'y': NewY--; NewX--; break;
        case '9': case 'u': NewY--; NewX++; break;
        case '1': case 'b': NewY++; NewX--; break;
        case '3': case 'n': NewY++; NewX++; break;

        case 't': {
          clear();

          #define MAX_TRAINERS 1000
          char symbols[MAX_TRAINERS];
          int rY[MAX_TRAINERS];
          int rX[MAX_TRAINERS];
          int count = 0;
          ...
          for (int i = 0; i < MAP_Y; i++) {
            for (int j = 0; j < MAP_X; j++) {
              if (world.cur_map->cmap[i][j] &&
                  world.cur_map->cmap[i][j]->symbol != '@') {

                symbols[count] = world.cur_map->cmap[i][j]->symbol;
                rY[count] = i - c->pos[dim_y];
                rX[count] = j - c->pos[dim_x];
                count++;
              }
            }
          }
          ...
          while (1) {
            ...
            for (int i = 0; i < max_lines && (start + i) < count; i++) {
              int idx = start + i;
              int y = rY[idx];
              int x = rX[idx];
              ...
              if (y == 0) {
                ...
                mvprintw(i, 0, "%c, %d %s and %d %s",
                         symbols[idx],
                         abs(y), y < 0 ? "north" : "south",
                         abs(x), x < 0 ? "west" : "east");
              }
            }

            mvprintw(22, 0, "Use UP/DOWN arrows to scroll, ESC to exit");
            refresh();

            ch = getch();

            if (ch == 27) break;
            else if ((ch == KEY_UP || ch == 'k') && start > 0) start--;
            else if ((ch == KEY_DOWN || ch == 'j') && start + max_lines < count) start++;
          }
      ...
      if (NewY >= 0 && NewY < MAP_Y &&
          NewX >= 0 && NewX < MAP_X) {
        if (world.cur_map->cmap[NewY][NewX] &&
            world.cur_map->cmap[NewY][NewX] != &world.pc) {

          character_t *enemy = world.cur_map->cmap[NewY][NewX];

          clear();

          char *type;
          switch (enemy->symbol) {
            case 'h': type = "Hiker"; break;
            case 'r': type = "Rival"; break;
            case 'p': type = "Pacer"; break;
            case 'w': type = "Wanderer"; break;
            case 's': type = "Sentry"; break;
            case 'e': type = "Explorer"; break;
            default:  type = "Trainer"; break;
          }

          mvprintw(10, 10, "You encountered a %s (%c)!", type, enemy->symbol);
          mvprintw(12, 10, "Press ESC to continue");
          refresh();

          while (getch() != 27);

          world.cur_map->cmap[NewY][NewX] = NULL;
        }

        terrain_type_t t = world.cur_map->map[NewY][NewX];
      ...
      int NewY = c->pos[dim_y] + dy[dir];
      int NewX = c->pos[dim_x] + dx[dir];
      ...
      c->next_turn += move_cost[char_other]
        [world.cur_map->map[c->pos[dim_y]][c->pos[dim_x]]];
...
//     if (ch == 27) {   // ESC
//       break;
//     } else if ((ch == KEY_UP || ch == 'k') && start > 0) {
//       start--;
//     } else if ((ch == KEY_DOWN || ch == 'j') && start + max_lines < count) {
//       start++;
//     }
//   }
...
//             clear();
//             mvprintw(10, 10, "Welcome to the Pokemart!");
//             mvprintw(12, 10, "Press < to leave");
//             refresh();
//             while (getch() != '<')
//               ;
//           } else if (world.cur_map->map[c->pos[dim_y]][c->pos[dim_x]] == ter_center) {
//             clear();
//             mvprintw(10, 10, "Welcome to the Pokemon Center!");
//             mvprintw(12, 10, "Press < to leave");
//             refresh();
//             while (getch() != '<')
//               ;
...
//       if (world.cur_map->cmap[NewY][NewX] &&
//           world.cur_map->cmap[NewY][NewX]->symbol != '@') {
//         clear();
//         mvprintw(10, 10, "A wild trainer appears!");
//         mvprintw(12, 10, "Press ESC to exit the battle");
//         refresh();
//         //int ch;
//         world.cur_map->cmap[NewY][NewX] = NULL;
//       } else {
